...
%DOIs are part of the scientific article publication process. A good DOI curation provides standard metadata that can be harvested and reused to generate citation with any information required for a BibTeX. It mays also include references and related products that build step by step a graph of linked resources.
...
\begin{enumerate}
	...
	%Users expect also a visible BibTeX in the Data Center web pages.
	%redo (Edwin)
	...
	\item 
	%A Workflow needs to execute again a process that includes external resources. 
...
Table \ref{tab:astroDOIs1} lists a short selection of assigned by Data Centers. The list is not exhaustive but shows different usage of DOI  \todo{complete the DOI table?}

%This list is not exhaustive; see Table 1 in Appendix A for a complete listing. 
...
%\todo{Gilles: I propose to remove the paragraph from here}The landing page provides one or more links to data.
...
%These infrastructures adopt merge mechanisms (for instance OpenAire\footnote{\url{todefine}}), often a black box, that depends of their own strategy.
\hl{These indexing services group together the information relating to the
	...
	indexing service's strategy.} % ask by Edwin
...
\hl{We use the ADS BibTex type "@dataset". This term is specific to ADS and is not used by service such as DataCite which uses the generic term "@misc".} % moved+change from Edwin ask
...
\todo{Gilles: I don't know if there is something to add here?
...
\todo{Gilles: may be hide the DOI and provide anonymous links?}
...
%\paragraph{Example}: EOSC EU node provides a portal that proviledges DOI.
%It select records having DOI from EUDAT which includes VO Registry records. The relationships using DOI are also used.
...
%Generaly, the description is not exploited by search process. However, like title,
