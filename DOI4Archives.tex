...
\newcommand{\dataciteterm}[1]{\textcolor{blue}{\mbox{datacite:\textbf{#1}}}}
\newcommand{\datacitetag}[2]{\href{\datacitepropurl{#1}}{\textcolor{blue}{\mbox{datacite:\textbf{#2}}}}}

%\href{https://datacite-metadata-schema.readthedocs.io/en/4.6/properties/date/#a-datetype}{DataCite:updated}
%\newcommand{\important}[1]{
%	\begin{bigdescription}
%		\item \color{ivoacolor}{Note} #1
%	\end{bigdescription}
%}
\newcommand{\important}[1]{\textbf{\color{ivoacolor}Note #1 \\ }}
...
\author[https://orcid.org/0000-0001-5982-167X]{Tim Jenness}
...
%		\textbf{\color{red} WARNING: this document is a DRAFT, and is not yet official.}\\
...
The architecture is robust and has been generalized to identify other types of data than those mentioned above. For example, DOIs are used for instruments (\href{https://rubinobservatory.org/}{Rubin Observatory}), and the \href{https://datacite.org/}{DataCite} organization which provides DOI for datasets, is a partner of \href{https://ardc.edu.au/}{ARDC} to register identifiers for projects (\href{https://support.datacite.org/docs/raids}{RAID}).
...
The richness of the DOI metadata is a valuable basis for implementing web search engines and drawing knowledge graphs, thus improving the discovery and traceability of digital resources.
...
\paragraph{Result sets}\label{sec:resultsets}

A Result set is the result returned by a query. It is the result of a service, a user request and datasets executed at a precise moment, which may be not reproducible if it involves evolving data. Thus, certain services such as the \href{https://vamdc.org/}{VAMDC} or \href{https://www.canfar.net/}{Canfar} provide a Query Store with DOI creation (Resultset have not been treated in the scope of this Document).

\subsubsection{Collections}\hypertarget{collections}{}\label{sec:collections}

The collection-type DOI directs users to a collection of individual data records that are related to each other (e.g. : individual observations from the same survey or archive, etc.). The collection provides access to resources (identified with a DOI or with another identifier) via relation (see  \ref{sec:a-basic-tour-of-relationship-motivating-by-their-usage}) defined in the DataCite schema \citep{https://doi.org/10.14454/mzv1-5b55}

Examples of collection-type DOIs include the DOI services provided by the Mikulski Archive for Space Telescopes (MAST) \citep{2018ApJS..236...20N} and the Chandra Data Archive (CDA) \citep{2018EPJWC.18612011R}. There is significant variation in the expected use cases for these Collection DOIs. There are also strong variations in the metadata of Collection-type DOIs.

Considering use-cases, both MAST and Chandra Collection DOIs collect dataset identifiers within their respective databases. For instance, Chandra Collection is made of DOIs of different types. Its metadata and related identifiers are defined to make a Chandra collection DOI the most versatile and general aggregation that can be obtained using Chandra data ``entities''.
...
In any case, \textbf{Collection-type DOI means a collection of resources accessible via sustainable URIs.}

\subsubsection{Service}\hypertarget{service}{}\label{sec:service}

A service DOI  designates both a set of services accessible via the web and the whole organizational system, including staff, necessary for its operation.
...
A knowledgebase is not a particular entity in the DOIs schemas ; it can be a dataset or a service depending on what has to be described (see \hyperlink{assigning-resource-type}{Assigning resource type}).
...
Once a DOI is published it must always be resolvable. If a resource with a DOI is removed from circulation (a data set is offlined, for example, or a digital document is accidentally destroyed) then the DOI must resolve to a tombstone page. A tombstone page (see \hyperlink{tombstones}{Tombstones} section) indicates clearly that the resource is no longer available, and usually offers contact information or an option for users who are attempting to access on the now-defunct resource. The tombstone page must remain as a legacy for as long as there is a DOI Foundation to resolve it.
...
RAs, such as DataCite or Crossref, provides Metadata schema, formats, and DOI submission modalities. The RA generally provides a form-based interface and APIs covering the registering protocols and metadata formats (such as JSON or XML). Metadata schema, formats, and DOI submission modalities are fixed by the RA.% The RA generally provides a form-based interface and APIs covering the registering protocols and metadata formats (such as JSON or XML)

For DOI creators minting small numbers of DOIs (a few dozen per year, say), the RA may provide a form-based interface for creating DOIs that guides the user through the most commonly used metadata attributes. \mbox{DataCite}, for example, has its \href{https://doi.datacite.org/}{Fabrica} interface for interactive DOI creation. (It can also be used to edit existing DOI metadata, one record at a time.)
...
Sometimes, RA provides a test and production environment. In \mbox{DataCite}, the ``\href{https://doi.test.datacite.org/}{test Fabrica}'' is a production clone that emulates the DOI generation. It is common for DataCenter who develops a DOI workflow.
...
Versioning allows to access the landing page for each versions. Each version has its own DOI and is linked to the other versions.
...
Once these baselines are met, there are other curational chores to perform as resources allow. Some organizations, for example, actively track citations of their products and add the information to their own DOI metadata  (see section \hyperlink{term_to_link_resources_with_their_origin}{Term to link resources with their origin}). Also, many archives are now thinking seriously about "metadata enrichment" - adding metadata to both standardize and improve the quality and success of searching, accessing, and (re)using their data holdings.
...
\hline
Table or set of tables & Dataset & 
    Observation datasets \newline
  	Compilation \newline
  	Evolved datasets \newline
    \href{https://www.ivoa.net/rdf/product-type/2024-05-19/product-type.html}{See "product-type" IVOA vocabulary}
	\\ \hline
Resource such as a table available in VO & Dataset & 
    IVOA resource \newline
    ObsCore table \newline
    EpnCore table\\\hline
Individual image & Image & 
    ...
    Image 3D \newline
    Sky survey\\ \hline
Collection of images & Dataset & 
    Images \newline
    curated images\\ \hline
Collection of Spectra & Dataset & Spectra\\ \hline
Logs of observation & Dataset & Evolving Datasets\\ \hline
Collection of resources having their own PID & Collection & 
    (specify the resource type) \newline
    Collection of images \newline
    Collection of observations \\ \hline
Set/collection of resources & Dataset & \\ \hline
VO service & Service & 
    IVOA service\newline
    IVOA TAP service  \\ \hline
...
\paragraph{How to pick titles ?}
We give a non exhaustive list of suggestions to be considered to create a useful title:
...
The "Creator"  is a fundamental metadata which allows to credit persons or organizations.The creator list is used by citation generators who extract information from DOI (see \hyperlink{citation-extraction}{Citation extraction}).

In the case of article or data product associated to an article, the creators are generally the list of authors.  The good usage consists of adding all authors (and not only the first author) and to provide ORCID and affiliations each time they are known.

For Observations, possibly subject to complex processes, the creators are persons (eg: PI) or the organization responsible of the observation.
...
\begin{enumerate}
\item{} The creator takes a substantial part in  the data creation (imagine, design the Observation, calibrate, analyse, etc.)
...
\subsubsection{Funding}\hypertarget{fundings}{}\label{fundings}
...
\subsubsection{A curated DOI is mandatory for extracting a citation}\hypertarget{curate-doi-is-mandatory-to-extract-citation}{}\label{sec:curate-doi-is-mandatory-to-extract-citation}
...
The solution may become complex if we take into account the reproducibility aspect or not. A simple solution without reproducibility constraint consists, in an article, to cite the evolving Dataset and to specify the query outside the citation, somewhere in the text.\newline
...
The list of metadata tends to increase, but the most popular are DataCite schema, \href{https://www.dublincore.org/}{Dublin Core}, the registry of the Virtual Observatory \citep{std:registry}, \href{https://schema.org/}{schema.org} (extends Dublin Core and is used by Google), \href{https://www.w3.org/TR/vocab-dcat-3/}{DCAT} (linked catalogues, datasets and services. DCAT is a concurrent of the VO registry), \href{https://opencitations.net/}{OpenCitation} \citep{Peroni_2020} (a schema of linked citation), etc.
...
The choice of identifier affects its reusability. For instance, the bibcode which is available in Datacite schema but is not recognized in OpenAire \footnote{\url{https://guidelines.openaire.eu/en/latest/data/field_identifier.html}} has a limited use in each service depending of OpenAire.

\important{To facilitate the usage, the DOI has to be privileged than any other identifier or URI.}


\subsubsection{Term to link resources with their origin}
\hypertarget{term_to_link_resources_with_their_origin}{}\label{sec:term_to_link_resources_with_their_origin}
...
VO Registry and DataCite schemas are based on DublinCore terms extended with their own specifications. Despite a common basis, the vocabularies differ. The DataOrigin note provides in Appendix a mapping between DataCite and VOResource for a sub selection of terms. 
%Note that the endorsement process of DataOrigin has not been initiated.
...
\emph{Note: at the time of writing this note, the EOSC EU nodes harvests the IVOA registry but makes a selection. Only resource with DOI are selected. }
...
It includes both, misunderstanding of the schema (for instance incorrect resourceType that happens in particular with Collections) and uncurated data (for instance when the information does not match the schema definition. eg: \textless{}creator\textgreater{}Henneken et al. (2024)\textless{}/creator\textgreater{}).
...
The ESAC Science Data Centre (ESDC) is the European Space Agency’s main hub for archiving, preserving, and providing access to space‑science data. It is located at the European Space Astronomy Centre (ESAC) near Madrid, Spain, and supports virtually all ESA missions in astronomy, planetary science, and heliophysics. ESA is providing DOIs for all the datasets from its space science missions and they can all be accessible at \url{https://www.cosmos.esa.int/web/esdc/doi}.



\paragraph{Links for examples of landing page and DOI}:\\
Gaia DR3 DOI landing page: \url{https://esdcdoi.esac.esa.int/doi/html/data/astronomy/gaia/DR3.html} \\
DOI for the fourth XMM-Newton serendipitous source catalogue: \url{https://doi.org/10.57780/esa-y9l9jwg} 
